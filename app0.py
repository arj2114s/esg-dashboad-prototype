...
with st.sidebar:
    ...
    total_target = st.number_input("Total Daily Target (Tons)", value=8000, step=200)
...
with c3:
    m3_dist = st.number_input("M3 Distance (km)", value=35)
...
with tab4:
    ...
    st.markdown("Visualizing the 24-hour daily trade-off between Fleet Cost, Emissions, and Operational Time.")
    
    F = s1_opt.F
    if len(F) > 0:
        # Scale the single-shift Pareto front by 3 to represent the full 24-hour daily totals
        daily_F = F * 3
        
        # CONVERT TO INTEGER DATAFRAME FOR DISCRETE READABILITY
        pareto_df = pd.DataFrame(daily_F, columns=["Cost", "CO2", "Time"]).astype(int)
        ...
        fig_3d = px.scatter_3d(
            ...
            title="Projected Daily Pareto Front (Cost vs CO2 vs Time)"
        )
        ...
        fig_2d = px.scatter(
            ...
            labels={"Cost": "Daily Cost (₹)", "CO2": "Daily CO2 (kg)", "Time": "Daily Time (hrs)"}
        )
        ...
        fig_par = px.parallel_coordinates(
            ...
            labels={"Cost": "Daily Cost (₹)", "CO2": "Daily CO2 (kg)", "Time": "Daily Time (h)"},
            color_continuous_scale=px.colors.diverging.Tealrose
        )
        # Margin fix to prevent cutting off labels
